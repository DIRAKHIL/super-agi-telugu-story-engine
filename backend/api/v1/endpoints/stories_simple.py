"""
Simple story generation API endpoints without orchestrator dependency
"""
import uuid
import asyncio
...
async def generate_story(request: StoryGenerationRequest, background_tasks: BackgroundTasks):
    """Generate a Telugu story based on the provided parameters"""
    try:
...
async def process_story_generation(task_id: str, request: StoryRequest):
    ...
    try:
        # Generate story
        result = await telugu_story_generator.generate_story(request)
        ...
        logger.info(f"Story generation task {task_id} completed successfully")
        
    except Exception as e:
        logger.error(f"Error processing story generation task {task_id}: {str(e)}")
